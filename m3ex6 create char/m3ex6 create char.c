...
void lcdCreateChar(uint8_t slot, uint8_t *charmap) {
    int i;
    // slot seja apenas de 0 a 7
    slot &= 0x07; 
    
    // Comando para o LCD ir na memória CGRAM na posição do slot escolhido
    lcdWriteByte(0x40 | (slot << 3), 0);

    // Escreve as 8 linhas de pixels na memória
    for (i = 0; i < 8; i++) {
        lcdWriteByte(charmap[i], 1);
    }

    lcdWriteByte(0x80, 0); // Comando para retornar o cursor para o início da tela
}
